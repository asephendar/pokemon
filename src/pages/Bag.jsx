...
const BerryList = () => {
        ...
        const fetchBerries = async (page) => {
            try {
                ...
                const response = await axios.get(
                    `https://pokeapi.co/api/v2/berry?limit=${pageSize}&offset=${offset}`
                );
    ...
    const handleShowModal = async (berry) => {
        try {
            ...
            const effectEntry = itemDetails.data.effect_entries.find(
                (entry) => entry.language.name === "en"
            ) || { effect: "No effect", short_effect: "No short effect" };
            setSelectedBerry({
                ...berry,
    ...
    return (
        <>
            ...
            <div className="container mx-auto">
                <Title Tag="h2" variant="text-3xl font-bold mb-4">
                    Bag
                </Title>
                        ...
                        <li key={index} className="card shadow-lg compact side bg-base-100">
                            <div className="card-body flex justify-between items-center">
                                <div className="flex items-center">
                                    <Figure
                                        image={berry.item.image}
                                        name={berry.name}
                                        variant="w-10 h-10 mr-2"
                                    ></Figure>
                                    <Title Tag="span" variant="card-title">
                                        {berry.name}
                                    </Title>
                                    <span className="ml-2 text-gray-500">
                                        ...
                                        <Eye
                                            ...
                                            variant="btn btn-ghost btn-xs mx-1"
                                        >
                                            👁
                                        </Eye>
                                    </span>
                ...
                <Title Tag="h1" variant="text-3xl font-bold my-6">
                    Berry List
                </Title>
                {alertMessage && (
                                    ...
                                    {berryParty.some((b) => b.name === berry.name) ? (
                                        <div className="flex items-center space-x-2">
                                            <button className="btn btn-sm" disabled>
                                                In the Bag (
                                                {berryParty.find((b) => b.name === berry.name)?.count ||
                                                    0}
                                                )
                                            </button>
                                            {berryParty.find((b) => b.name === berry.name)?.count <
                ...
                {showModal && selectedBerry && (
                    <div className="modal modal-open">
                        <div className="modal-box">
                            <form method="dialog">
                                <Button
                                    pokemon={selectedBerry}
                                    variant="btn btn-circle btn-ghost absolute right-2 top-2"
                                    onClick={handleCloseModal}
                                >
                                    ✕
                                </Button>
                            </form>
                            <Title Tag="h3" variant="font-bold text-lg mb-1">
                                {selectedBerry.name}
                            </Title>
                            <Figure
                                image={selectedBerry.item.image}
                                name={selectedBerry.name}
                                variant=""
                            ></Figure>
                            <Title Tag="p" variant="py-4">
                                {selectedBerry.effect}
                            </Title>
                            <Title Tag="p" variant="">
                                {selectedBerry.short_effect}
                            </Title>
                        </div>
                    </div>
                )}
